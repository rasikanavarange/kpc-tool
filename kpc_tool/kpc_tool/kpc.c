...
/// Profile a particular process.
/// @param target_pid Target process pid, -1 for all threads.
/// @param total_profile_time Profile time in seconds.
/// @param sample_period Profile sampler period in seconds.
...
int profile_process(const int target_pid, const double total_profile_time, const double sample_period) {
    ...
    // load dylib
    if (!lib_init()) {
        printf("Error: Failed to load kperf/kperfdata\n");
        return 1;
    }
    ...
    printf("target pid: %d\n", target_pid);
    printf("total profile time: %g seconds\n", total_profile_time);
    printf("sample period: %g seconds\n", sample_period);
    ...
    for (usize i = 0; i < ev_count; i++) {
        ...
        if ((ret = kpep_config_add_event(cfg, &ev, 1, NULL))) {
